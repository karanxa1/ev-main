...
import './ChangeVehiclePage.css'; // Path will be correct as it's a sibling
...
const ChangeVehiclePage = () => {
  ...
  // Fetch current vehicle on mount
  useEffect(() => {
      ...
      if (currentUser) {
        ...
        try {
          ...
          if (userDoc.exists() && userDoc.data().vehicle) {
            const currentVehicleData = userDoc.data().vehicle;
            // Find the full vehicle object from processedVehicles to set it correctly
            const fullVehicleObject = processedVehicles.find(v => v.id === currentVehicleData.id);
            if (fullVehicleObject) {
              setSelectedVehicle(fullVehicleObject);
              // Optionally, set brand tab if needed
              // setSelectedBrand(fullVehicleObject.brand);
            } else {
              // If the stored vehicle ID doesn't match any in our list (e.g. old/invalid data)
              // We might want to clear it or handle it. For now, just log.
              console.warn("Stored vehicle ID not found in current vehicle list:", currentVehicleData.id);
              setSelectedVehicle(currentVehicleData); // Fallback to stored data, might lack some UI properties
            }
            console.log("Current vehicle loaded:", currentVehicleData);
          }
        } catch (err) {
          console.error("Error fetching current vehicle:", err);
          setError("Could not load your current vehicle.");
        }
        setLoading(false);
      }
    };
  ...
  }, [currentUser]); // Removed processedVehicles from deps to avoid re-fetch on its change, as it should be stable
  ...
  const handleUpdateVehicle = async () => {
    ...
    try {
      if (!currentUser) {
        throw new Error('You must be logged in to update your vehicle');
      }
      ...
      // Ensure user document exists, then update
      const userDoc = await getDoc(userRef);
      if (!userDoc.exists()) {
        // This case should ideally not happen if user is logged in and has accessed this page
        // But as a safeguard, we can create the doc or error out
        console.log('User document not found, creating with new vehicle...');
        ...
        console.log('Updating user document with new vehicle data...');
        await updateDoc(userRef, {
          vehicle: vehicleData
        });
      }

      console.log('Vehicle updated in profile:', selectedVehicle.name);
    ...
    <div className="select-vehicle-page">
      ...
      <main className="svp-vehicle-grid">
        {filteredVehicles.length > 0 ? (
          ...
          <p className="svp-no-results">No vehicles match your criteria. Try a different brand or search term.</p>
        )}
      </main>
      ...
      <footer className="svp-footer">
        ...
        >
          {loading ? 'Updating Vehicle...' : 'Update My Vehicle'}
        </button>
      </footer>
    </div>
  );
